...
(defn signup-form
  [{:keys [recaptcha/site-key params]}]
  (biff/form {:id "signup"
              :action "/auth/send-link"
              :hidden {:on-error "/"}
              :class "sm:max-w-ws w-full"}
               ...
               [:<>
                ...
                [:.text-sm.text-red-600
                 (case error
                   ...
                   "invaild-email" "Invalid email. Try again with a different address"
                   "send-failed" (str "We were not able to send an email to that address. "
